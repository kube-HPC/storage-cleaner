...
class ResultsCleaner extends BaseCleaner {
    async clean() {
        try {
            ...
            for (let jobId of jobsToDelete) { // eslint-disable-line
                const promiseArray = [];
                deletePromise.push(promiseWrapper(() => storageManager.hkube.delete({ jobId }))); // eslint-disable-line
                deletePromise.push(promiseWrapper(() => storageManager.hkubeMetadata.delete({ jobId }))); // eslint-disable-line
                deletePromise.push(promiseWrapper(() => storageManager.hkubeExecutions.delete({ jobId }))); // eslint-disable-line
                deletePromise.push(promiseWrapper(() => storageManager.hkubeResults.delete({ jobId }))); // eslint-disable-line
            ...
            expiredDates.forEach((date) => {
                indexesToDeletePromies.push(promiseWrapper(() => storageManager.hkubeIndex.delete({ date })));
            });
            const results = await Promise.all(indexesToDeletePromies);
            this._handleErrors(results);
        }
        catch (error) {
    ...
    _handleErrors(results) {
        ...
        if (errors.length) {
            this.log.info(`result-cleaner:failed to delete ${errors.length} objects`);
            errors.forEach((error) => {
                this.log.info(`result-cleaner:failed to delete ${error.message}`);
            });
        }
    }
}
